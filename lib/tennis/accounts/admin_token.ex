defmodule Tennis.Accounts.AdminToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "admins_tokens" do
    ...
    belongs_to :admin, Tennis.Accounts.Admin
  ...
  def build_session_token(admin) do
    ...
    {token, %Tennis.Accounts.AdminToken{token: token, context: "session", admin_id: admin.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Tennis.Accounts.AdminToken, where: [token: ^token, context: ^context]
  end
  ...
  def admin_and_contexts_query(admin, :all) do
    from t in Tennis.Accounts.AdminToken, where: t.admin_id == ^admin.id
  end

  def admin_and_contexts_query(admin, [_ | _] = contexts) do
    from t in Tennis.Accounts.AdminToken, where: t.admin_id == ^admin.id and t.context in ^contexts
  end
end
